...
async def scrape_marketplace(page, new_price):
    # try city-name URL first, fall back to coordinates
    urls = [
        f"https://www.facebook.com/marketplace/markham/search/?query={QUERY.replace(' ', '+')}",
        f"https://www.facebook.com/marketplace/search/?query={QUERY.replace(' ', '+')}&latitude={LAT}&longitude={LNG}&radius={RADIUS_KM}&exact=false",
    ]
    for url in urls:
    ...
    for a in anchors[:30]:
        try:
            ...
            # grab all visible text inside the card
            texts = []
            ...
            # title — longest text that isn't a price or very short
